"""Referral gating: run only for accounts signed up under a given code.

BULK's indexer exposes, for any wallet, which referral code and referrer wallet
it registered under:

    GET https://indexer.bulk.trade/v1/aura/wallet/<pubkey>
    -> {"referred_by_code": "VAULT", "referred_by_wallet": "GDMm...", ...}

Observed responses, which the checks below are built around:

    valid wallet, has a referrer   200, referred_by_* populated
    valid wallet, no referrer      200, referred_by_* are null
...
def check_access(
    ...
    *,
    ...
    if not status.has_referrer:
        return AccessDecision(
            False,
            f"{wallet} did not sign up under a referral code",
            status,
        )
    ...
    if status.referred_by_wallet and status.referred_by_wallet in set(config.wallets):
        return AccessDecision(
    ...
    return AccessDecision(
        False,
        f"{wallet} signed up under {status.referred_by_code or status.referred_by_wallet}, "
